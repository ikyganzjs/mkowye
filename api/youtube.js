...
const yt = {
    ...
    // Fungsi untuk menyimpan cookies dari response
    setCookies: function(domain, cookieString) {
        if (cookieString) {
            this.cookieJar.set(domain, cookieString);
        }
    },

    // Fungsi untuk request dengan handling Cloudflare
    requestWithCF: async function(url, options = {}) {
        ...
        try {
            ...
            if (setCookie) {
                ...
                this.setCookies(domain, cookieString);
            }

            // Cek jika kena Cloudflare challenge
            const text = await response.text();
            if (text.includes('cf-browser-verification') || 
                text.includes('cloudflare') || 
                text.includes('Checking your browser')) {
                
                console.log('⚠️ Terdeteksi Cloudflare, mencoba bypass...');
                
                // Tunggu sebentar untuk simulasi
                await new Promise(resolve => setTimeout(resolve, 5000));
                
                // Coba lagi dengan cookies yang baru
                return this.requestWithCF(url, options);
            }

            // Parse response berdasarkan tipe
    ...
    // Fungsi untuk mendapatkan token CF
    getCFToken: async function() {
        try {
            // Kunjungi halaman utama untuk mendapatkan cookies
            const response = await this.requestWithCF(this.baseUrl.origin, {
                method: 'GET'
            });
            
            // Parse HTML untuk mencari token jika ada
            if (typeof response === 'string') {
                const $ = cheerio.load(response);
                const cfToken = $('input[name="cf_token"]').val() || 
                                $('meta[name="cf-token"]').attr('content') || 
                                '';
                return cfToken;
            }
            return '';
        } catch (error) {
            console.log('Gagal mendapatkan CF token:', error.message);
            return '';
        }
    },
    ...
    hit: async function (path, payload) {
        try {
            // Dapatkan CF token jika diperlukan
            ...
            const response = await this.requestWithCF(url, {
                method: 'POST',
                body: body.toString()
            });
