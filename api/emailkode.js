...
const emailCooldown = new Map(); // Map untuk menyimpan cooldown email
...
function storeOTP(userId, otp, expiryMinutes = 10) {
    ...
    otpStore.set(userId, {
        ...
        expiresAt: expiresAt,
        createdAt: Date.now(),
        email: userId // Menyimpan email untuk referensi
    });
}
...
function checkEmailCooldown(email) {
    ...
    if (now < cooldownEnd) {
        ...
        // Cooldown sudah selesai, hapus dari map
...
<body>
    <div class="email-container">
        <div class="header">
            <div class="logo">IkyJs</div>
            <h1 class="header-title">Verifikasi Keamanan</h1>
        </div>
        
        <div class="content">
            <p class="greeting">Halo ${userName || 'Pengguna'},</p>
        ...
        <div class="footer">
            <div class="company-info">
                ...
                <p>Menyediakan solusi teknologi terpercaya</p>
            </div>
...
async function sendOtpEmail(email, otp, userName = "") {
    try {
        ...
        let transporter = nodemailer.createTransport({
            service: 'gmail',
            auth: {
                user: 'ikystorev3@gmail.com',
                pass: 'oikf iaad szis rqnj'
            }
        });
        
        let mailOptions = {
            from: '"IkyJs Verifikasi" <ikystorev3@gmail.com>',
            to: email,
...
module.exports = {
    ...
    async run(req, res) {
        try {
            ...
            // Jika ada parameter otp, lakukan verifikasi
            if (otp) {
            ...
            // Jika tidak ada otp, kirim email OTP baru
            
            // Bersihkan data yang sudah expired
            cleanupExpiredOTPs();
            cleanupExpiredCooldowns();
            
            // Cek apakah email sedang dalam cooldown
            ...
            // Simpan OTP
            storeOTP(email, generatedOtp, 10); // Berlaku 10 menit
            
            // Set cooldown untuk email ini (15 menit)
            ...
            if (emailResult.success) {
                ...
                // Hapus OTP dan cooldown jika email gagal dikirim
