...
const emailAccounts = [
    ...
    {
        email: 'ikyotp2@gmail.com',
        password: 'bcsk keys chae xghf'
    },
    {
        email: 'ikyotp3@gmail.com',
        password: 'oifp eugy uerl rifs'
    },
    {
        email: 'ikyotp4@gmail.com',
        password: 'xpgi dhtg ysfo gauz'
    },
    {
        email: 'ikyotp5@gmail.com',
        password: 'jept itnh fsdk mxht'
    }
];

// Cache untuk status email
const emailStatus = new Map();
const emailLocks = new Map(); // Untuk mencegah race condition

// Fungsi untuk memilih email secara random dengan locking mechanism
function getRandomEmailAccount() {
    const now = Date.now();
    
    // Filter email yang tersedia (tidak terkunci dan tidak sedang mengirim)
    const availableAccounts = emailAccounts.filter(account => {
        const status = emailStatus.get(account.email);
        const isLocked = emailLocks.get(account.email);
        
        // Hapus lock yang sudah expired (lebih dari 30 detik)
        if (isLocked && now - isLocked > 30000) {
            emailLocks.delete(account.email);
            return true;
        }
        ...
        // Jika tidak ada status atau tidak sedang mengirim
        if (!status || !status.isSending) {
            return true;
        }
        
        // Jika sedang mengirim tapi sudah lebih dari 1 menit
        if (status.isSending && now - status.lastUsed > 60000) {
            return true;
        }
    ...
    if (availableAccounts.length === 0) {
        ...
        let oldestAccount = emailAccounts[0];
        let oldestTime = Infinity;
        
        for (const account of emailAccounts) {
            const status = emailStatus.get(account.email);
            const lastUsed = status ? status.lastUsed : 0;
        ...
        // Set lock untuk email yang dipilih
        emailLocks.set(oldestAccount.email, now);
        updateEmailStatus(oldestAccount.email, true);
    ...
    // Set lock untuk mencegah email dipilih oleh request lain
    emailLocks.set(selectedAccount.email, now);
    updateEmailStatus(selectedAccount.email, true);
    
    return selectedAccount;
}

// Fungsi untuk update status email dengan auto-unlock
function updateEmailStatus(email, isSending = false) {
    ...
    // Jika selesai mengirim, hapus lock setelah beberapa saat
    if (!isSending) {
        setTimeout(() => {
            emailLocks.delete(email);
        }, 5000); // Hapus lock setelah 5 detik
    }
}
...
// Cleanup locks yang expired
function cleanupExpiredLocks() {
    const now = Date.now();
    for (const [email, lockTime] of emailLocks.entries()) {
...
async function sendOtpEmail(email, otp, userName = "") {
    try {
        ...
        // Lepaskan lock jika ada error
        if (emailAccount) {
            emailLocks.delete(emailAccount.email);
            updateEmailStatus(emailAccount.email, false);
        }
...
module.exports = {
    ...
    async run(req, res) {
        try {
            ...
            // FIX: Validasi API key yang benar
            if (apikey !== '2025Prem') {
                return res.json({ 
                    status: false, 
