...
// Daftar 5 email dengan credentials masing-masing
const emailAccounts = [
    {
...
function getRandomEmailAccount() {
    const availableAccounts = emailAccounts.filter(account => {
        ...
        return !status || !status.isSending || Date.now() - status.lastUsed > 60000; // 1 menit cooldown
    });
    
    if (availableAccounts.length === 0) {
        // Jika semua sedang digunakan, pilih yang sudah lama tidak digunakan
        const now = Date.now();
        return emailAccounts.reduce((oldest, current) => {
            const currentStatus = emailStatus.get(current.email) || { lastUsed: 0 };
            const oldestStatus = emailStatus.get(oldest.email) || { lastUsed: 0 };
            return currentStatus.lastUsed < oldestStatus.lastUsed ? current : oldest;
        });
    }
...
// Fungsi utama untuk mengirim OTP (TIDAK DIUBAH)
